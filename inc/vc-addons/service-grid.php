...
add_shortcode( 'builder_service_grid', function ( $atts, $content = null ) {
    ...
    $btn_text       = ( isset( $btn_text ) && '' != $btn_text ) ? $btn_text : __( 'Learn more', 'builder-core' );
    var_dump( $isset_btn );
    //variables end
    ...
    while ( $service_query->have_posts() ): $service_query->the_post();
        ...
        if ( $isset_btn == 1 ) {
            $output .= '<a href="' . get_the_permalink() . '" class="btn btn-service hvr-sweep-to-right">' . esc_html( $btn_text ) . '</a>';
        }
        $output .= '</div>';
...
function builder_service_grid() {
    ...
    vc_map( array(
        ...
        "params"   => array(
            ...
            array(
                "type"       => "dropdown",
                "param_name" => "isset_btn",
                "value"      => array(
                    'Yes' => 1,
                    'No'  => 0,
                ),
                "heading"    => __( "Display Button", "builder-core" ),
                "std"        => "Yes",
                "group"      => "General",
            ),
            array(
